...
public abstract class JDBCoDataServiceForSchema extends JDBCoDataService {
	...
	public Response getODataEntitySetForSchema(
			@Parameter(description = "schemaname", example = "INFORMATION_SCHEMA") String schemaraw,
			@Parameter(description = "objectname", example = "USERS") String nameraw,
			@Parameter(description = "The list of columns to return, the projection in more general terms", example = "USERNAME, ACTIVE") String select,
			@Parameter(description = "An OData filter condition", example = "") String filter,
			@Parameter(description = "An OData order by clause", example = "USERNAME") String order,
			@Parameter(description = "Limit to n many records", example = "100") Integer top,
			@Parameter(description = "Skip the first n records for pagination", example = "0") Integer skip,
			@Parameter(description = "Skip token for pagination", example = "") String skiptoken,
			@Parameter(description = "Optional parameter to overrule the format", example = "json") String format) {
		try {
			String schema = ODataUtils.decodeName(schemaraw);
			String name = ODataUtils.decodeName(nameraw);
			ODataIdentifier identifier = createODataIdentifier(schema, name);
			return getEntitySetImpl(schemaraw, nameraw, select, filter, order, top, skip, skiptoken, format,
					identifier);
		} catch (Exception e) {
			ODataError error = new ODataError(e);
			return createResponse(error.getStatusCode(), error, format, request);
	...
	public Response getODataEntityRowForSchema(
			@Parameter(description = "schemaname", example = "INFORMATION_SCHEMA") String schemaraw,
			@Parameter(description = "objectname", example = "USERS") String nameraw,
			@Parameter(description = "A text with either a single value - in case the primary key consists of a single column only - or comma separated list of key=value components", example = "PUBLIC") String keys,
			@Parameter(description = "The list of columns to return, the projection in more general terms", example = "USERNAME, ACTIVE") String select,
			@Parameter(description = "Optional parameter to overrule the format", example = "json") String format) {
		try {
			try (Connection conn = getConnection();) {
